    ...
    while (ctx.arg_i < args.len) : (ctx.arg_i += 1) {
        ...
        if (utils.startsWith(arg, "--")) {
            inline for (info.fields) |field| {
                ...
                if (utils.splitAtFirst(argName, '=')) |parts| {
                    argName = parts[0];
                    next = parts[1];
                }
                if (mem.eql(u8, argName, name)) {
                    const typeInfo = @typeInfo(utils.NonOptional(field.field_type));
                    switch (typeInfo) {
                        .Bool => {
                        ...
                        .Pointer => |pointerType| {
                            ...
                            if (pointerType.is_const) {
                                ...
                                var slice = comptime if (utils.isOptional(field.field_type))
...
fn parseValueForField(
    ...
    value: []const u8,
) ArgParseError!utils.NonOptional(FieldType) {
    const FT = utils.NonOptional(FieldType);
